...
def DatabaseCreation():
    database_cursor.execute('CREATE TABLE IF NOT EXISTS Book(id INTEGER PRIMARY KEY AUTOINCREMENT,title VARCHAR(200),author VARCHAR(255),description VARCHAR(500))')
DatabaseCreation()
...
def UserBooksView(book_id=None):
    list_of_ids_and_titles = database_cursor.execute('SELECT id,title FROM Book').fetchall()
    if book_id != None:
        book_details = database_cursor.execute('SELECT * FROM Book WHERE id = {}'.format(book_id)).fetchone()
        return book_details   
...
def EditBook(book_id=None,title=None,author=None,description=None):
    ...
    database_cursor.execute('UPDATE Book SET title= ?,author= ?,description= ? WHERE id= ?',data_to_edit)
    database.commit()
...
def SaveNewBookToDatabase(title=None,author=None,description=None):
    ...
    database_cursor.execute('INSERT INTO Book(title,author,description) VALUES (?,?,?)',data_to_save)
    database.commit()
...
def SearchForBook(keyword=None):
    result = database_cursor.execute("SELECT * FROM Book WHERE title like '%{}%'".format(keyword)).fetchall()
    return result
